...
\begin{paper}
	Now let’s, well, turn to another verb, \bh{på̄nå̄}, which has the same root of the noun \bh{på̄nīm} ‘a face’. It is translated mostly using \C{troi} ‘to turn’ or \C{edrych} (comparable here with English \textit{to face} to a certain degree). This distinction is not expressed in the original Hebrew text lexically, and Morgan had to choose in each occurance of \bh{på̄nå̄} whether to use the one or the other, or in fewer cases a completely different verb.
\end{paper}
...
\begin{paper}
	{\click} If \bh{rå̄ʾå̄} and \bh{på̄nå̄} show limited lexical distribution to two main lexemes each in the translation, and \bh{hibbīṭ} and \bh{ḥå̄zå̄} show uniformity, \bh{ṣå̄p̄å̄} ‘to watch’ and \bh{šå̄r} ‘to behold’ is more finely tuned according to meaning; for example \C{disgwil} is used in the sense of anticipation (\exvref{Lam.}{4}{17}{}) and negated \C{cael} is added to \C{gweled} when describing inability to see because of concealment (\exvref{Job}{24}{15}{}).
...
\begin{paper}
	\bh{hišqīp̄} ‘to view, to look from above’ is generally translated using \C{edrych}, which is expectable. Three occurances of impersonal \C{gwelir} translates the \bh{nišqå̄p̄}, which is morphologically a passive form although is also used actively.
\end{paper}
...
\begin{hopoint}
	\begin{compactitem}
		\item Most show simple conversion, with very little or no variation:
			\bh{hɛrʾå̄}→\C{dangos} and \bh{nirʾå̄}→\C{ymddangos} (the causative and passive of \bh{rå̄ʾå̄}), \bh{hibbīṭ}→\C{edrych}, \bh{ḥå̄zå̄}→\C{gweled}, \bh{hišqīp̄}→\C{edrych}, \bh{hišgīaḥ}→\C{edrych}, \bh{rōʾɛ}→\C{gweledudd} and \bh{ḥōzɛ}→\C{gweledudd}.
